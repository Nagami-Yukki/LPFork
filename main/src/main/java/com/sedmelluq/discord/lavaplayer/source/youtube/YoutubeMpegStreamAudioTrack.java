...
/**
 * YouTube segmented MPEG stream track. The base URL always gives the latest
 * chunk. Every chunk contains the current
 * sequence number in it, which is used to get the sequence number of the next
 * segment. This is repeated until YouTube
...
public class YoutubeMpegStreamAudioTrack extends MpegAudioTrack {
  ...
  private static final RequestConfig streamingRequestConfig = RequestConfig.custom().setSocketTimeout(2000)
  ...
  public YoutubeMpegStreamAudioTrack(AudioTrackInfo trackInfo, HttpInterface httpInterface, URI signedUrl) {
    ...
    // YouTube does not return a segment until it is ready, this might trigger a
    // connect timeout otherwise.
  ...
  private void processNextSegmentWithRetry(
      ...
      TrackState state) throws InterruptedException {
    if (processNextSegment(localExecutor, state)) {
      return;
    }

    // First attempt gave empty result, possibly because the stream is not yet
    // finished, but the next segment is just
    // not ready yet. Keep retrying at EMPTY_RETRY_INTERVAL_MS intervals until
    // EMPTY_RETRY_THRESHOLD_MS is reached.
    long waitStart = System.currentTimeMillis();
    long iterationStart = waitStart;

    while (!processNextSegment(localExecutor, state)) {
      // EMPTY_RETRY_THRESHOLD_MS is the maximum time between the end of the first
      // attempt and the beginning of the last
  ...
  private boolean processNextSegment(
    ...
    try (YoutubePersistentHttpStream stream = new YoutubePersistentHttpStream(httpInterface, segmentUrl,
        CONTENT_LENGTH_UNKNOWN)) {
      if (stream.checkStatusCode() == HttpStatus.SC_NO_CONTENT || stream.getContentLength() == 0) {
        return false;
      }

      // If we were redirected, use that URL as a base for the next segment URL.
      // Otherwise we will likely get redirected
      // again on every other request, which is inefficient (redirects across domains,
      // the original URL is always
  ...
  private void processSegmentStream(SeekableInputStream stream, AudioProcessingContext context, TrackState state)
